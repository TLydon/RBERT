...
create_optimizer <- function(loss,
  ...
  # Return and create (if necessary) the global step tensor.
  # This is used for keeping track of total number of training steps.
  global_step <- tensorflow::tf$compat$v1$train$get_or_create_global_step()
  ...
  # Implements linear decay of the learning rate.
  # https://devdocs.io/tensorflow~python/tf/train/polynomial_decay
  learning_rate <- tensorflow::tf$compat$v1$train$polynomial_decay(
    learning_rate,
    global_step,
    num_train_steps,
    end_learning_rate = 0.0,
  ...
  if (use_tpu) {
    optimizer <- tensorflow::tf$compat$v1$tpu$CrossShardOptimizer(optimizer)
  }
...
AdamWeightDecayOptimizer <- function(learning_rate,
  ...
  this_opt <- tensorflow::tf$compat$v1$train$Optimizer(use_locking = FALSE, name = name)
  class(this_opt) <- c("AdamWeightDecayOptimizer", class(this_opt))
