...
with(tensorflow::tf$compat$v1$variable_scope(scope, default_name = "bert"), {
with(tensorflow::tf$compat$v1$variable_scope("embeddings"), {
...
with(tensorflow::tf$compat$v1$variable_scope("encoder"), {
...
with(tensorflow::tf$compat$v1$variable_scope("pooler"), {
...
pooled_output <- tensorflow::tf$compat$v1$layers$dense(
first_token_tensor,
config$hidden_size,
...
#' )
#' })
#' tvars <- tensorflow::tf$compat$v1$get_collection(
#' tensorflow::tf$compat$v1$GraphKeys$GLOBAL_VARIABLES
#' )
#' temp_dir <- tempdir()
...
return(
tensorflow::tf$compat$v1$truncated_normal_initializer(stddev = initializer_range)
)
}
...
# `query_layer` = [B*F, N*H]
query_layer <- tensorflow::tf$compat$v1$layers$dense(
from_tensor_2d,
num_attention_heads * size_per_head,
...
# `key_layer` = [B*T, N*H]
key_layer <- tensorflow::tf$compat$v1$layers$dense(
to_tensor_2d,
num_attention_heads * size_per_head,
activation = key_act,
name = "key",
...
# `value_layer` = [B*T, N*H]
value_layer <- tensorflow::tf$compat$v1$layers$dense(
to_tensor_2d,
num_attention_heads * size_per_head,
...
with(tensorflow::tf$compat$v1$variable_scope(scope_name), {
layer_input <- prev_output
with(tensorflow::tf$compat$v1$variable_scope("attention"), {
...
# -JDB
with(tensorflow::tf$compat$v1$variable_scope("self"), {
...
with(tensorflow::tf$compat$v1$variable_scope("output"), {
attention_output <- tensorflow::tf$compat$v1$layers$dense(
attention_output,
hidden_size,
kernel_initializer = create_initializer(initializer_range)
)
...
with(tensorflow::tf$compat$v1$variable_scope("intermediate"), {
intermediate_output <- tensorflow::tf$compat$v1$layers$dense(
attention_output,
intermediate_size,
...
with(tensorflow::tf$compat$v1$variable_scope("output"), {
layer_output <- tensorflow::tf$compat$v1$layers$dense(
intermediate_output,
hidden_size,
kernel_initializer = create_initializer(initializer_range)
)
...
#' reuse = tensorflow::tf$compat$v1$AUTO_REUSE
#' ), {
#' phx <- tensorflow::tf$compat$v1$placeholder(tensorflow::tf$int32, shape = c(4))
