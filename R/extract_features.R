...
#' \code{tf$compat$v1$estimator$tpu$TPUEstimatorSpec} call.
#'
...
tvars <- tensorflow::tf$compat$v1$trainable_variables()
...
tensorflow::tf$compat$v1$train$init_from_checkpoint(
init_checkpoint,
assignment_map
)
return(tensorflow::tf$compat$v1$train$Scaffold())
}
...
output_spec <- tensorflow::tf$compat$v1$estimator$tpu$TPUEstimatorSpec(
mode = mode,
...
tpu_config = tensorflow::tf$compat$v1$estimator$tpu$TPUConfig(
num_shards = 8L,
...
estimator <- tensorflow::tf$compat$v1$estimator$tpu$TPUEstimator(
...
tensorflow::tf$compat$v1$logging$set_verbosity(old_verbosity_level)
